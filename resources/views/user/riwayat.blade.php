    ...
    <div class="space-y-6">
        @forelse($pemesanans as $order)
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="flex justify-between text-sm">
                                        ...
                                        <span class="font-bold text-gray-800">Rp {{ number_format($detail->penerbangan->harga, 0, ',', '.') }}</span>
                                    </div>
                                    @endforeach
            ...
            <div id="invoice-{{ $order->id }}" class="fixed inset-0 z-50 hidden overflow-y-auto">
                <div class="fixed inset-0 bg-black/60 backdrop-blur-sm transition-opacity" onclick="hideModal('invoice-{{ $order->id }}')"></div>
                <div class="flex items-center justify-center min-h-screen p-4">
                    <div class="relative bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden transform transition-all">
                        <div class="p-6 border-b border-gray-50 flex justify-between items-center bg-indigo-600 text-white">
                            ...
                            <button type="button" onclick="hideModal('invoice-{{ $order->id }}')" class="text-indigo-200 hover:text-white transition">
                                <i class="fas fa-times text-lg"></i>
                            </button>
                        </div>
                        ...
                        <div class="p-8 space-y-6">
                            <div class="flex justify-between items-start border-b border-dashed border-gray-200 pb-6">
                                <div>
                                    ...
                                    <h4 class="text-2xl font-black text-indigo-600">{{ $order->kode_pemesanan }}</h4>
                                    <p class="text-[10px] text-gray-400 mt-1 uppercase">{{ $order->waktu_pemesanan->format('d M Y, H:i') }}</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Status</p>
                                    <span class="inline-block px-3 py-1 mt-1 bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase rounded-lg border border-current">
                                    ...
                                    <div class="flex justify-between items-center bg-gray-50 p-3 rounded-xl border border-gray-100">
                                        <div>
                                            ...
                                            <p class="text-[10px] font-medium text-indigo-500 flex items-center gap-1">
                                                <i class="fas fa-id-card text-[9px]"></i> NIK: <span class="font-mono">{{ $detail->nik }}</span>
                                            </p>
                                        </div>
                                        <div class="text-right">
                            ...
                            @php $flightInfo = $order->details->first()->penerbangan; @endphp
                            <div class="bg-indigo-50/50 p-4 rounded-2xl space-y-3 border border-indigo-100">
                                <div class="flex justify-between items-center">
                                    <div class="text-left">
                                        <p class="text-[10px] font-bold text-gray-400 uppercase">Keberangkatan</p>
                                        <p class="text-base font-black text-indigo-700">{{ $flightInfo->waktu_berangkat->format('H:i') }}</p>
                                        <p class="text-sm font-black text-gray-800">{{ $flightInfo->bandaraAsal->kode_iata }}</p>
                                    </div>
                                    <div class="flex flex-col items-center flex-1 px-4">
                                        <span class="text-[9px] text-indigo-400 font-bold">{{ $flightInfo->durasi }}</span>
                                        <div class="flex items-center w-full">
                                            <div class="h-[1px] flex-1 bg-indigo-200"></div>
                                            <i class="fas fa-plane text-indigo-400 text-[10px] mx-2"></i>
                                            <div class="h-[1px] flex-1 bg-indigo-200"></div>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <p class="text-[10px] font-bold text-gray-400 uppercase">Kedatangan</p>
                                        <p class="text-base font-black text-indigo-700">{{ $flightInfo->waktu_datang->format('H:i') }}</p>
                                        <p class="text-sm font-black text-gray-800">{{ $flightInfo->bandaraTujuan->kode_iata }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
